...
import { Flex, Button, Input, Textarea, Image, Box, Spinner } from '@chakra-ui/react'; // Dodany Spinner dla ładowania
            ...
            entries.map((entry, index) => (
              <div key={index} className="category-card">
                ...
                > 
                  ...
                  <h3 className="category-title">{entry.title || 'Bez tytułu'}</h3>
                  <p className="category-description">{entry.body || 'Brak treści'}</p>
                  <p className="category-description">{entry.price || 'Brak ceny'}</p>
                </Box>
              </div>
            ))
          ) : (
