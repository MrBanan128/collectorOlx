...
router.get("/user-info", authMiddleware, async (req, res) => {
  try {
    ...
    res
      .status(500)
      .json({ message: "Błąd pobierania danych użytkownika", details: error });
  }
});
...
router.put(
  ...
  async (req, res) => {
    try {
      ...
      res
        .status(500)
        .json({ message: "Błąd podczas aktualizacji", details: error });
    }
  }
);
...
router.delete("/users", authMiddleware, async (req, res) => {
  try {
    ...
    res.json({
      message:
        "Konto użytkownika i wszystkie jego notatki oraz zdjęcia zostały usunięte"
    });
  } catch (error) {
    res
      .status(500)
      .json({ message: "Błąd podczas usuwania konta", details: error });
  }
});
...
router.get("/users/entries", authMiddleware, async (req, res) => {
  try {
    ...
    res
      .status(500)
      .json({ message: "Błąd pobierania notatek", details: error });
  }
});
...
router.get("/entries", async (req, res) => {
  try {
    const notes = await Note.find()
      ...
      .limit(12); // Ograniczenie do 12 elementów
...
router.get("/users/entries/:noteId", async (req, res) => {
  try {
    ...
    res
      .status(500)
      .json({ message: "Błąd pobierania notatki", details: error });
  }
});
...
router.put("/users/entries/:noteId", authMiddleware, async (req, res) => {
  try {
    const { noteId } = req.params;
    const { title, body, price, expertRequest, expertId, expertEvaluation } =
      req.body; // Dodajemy expertId
    ...
    res
      .status(500)
      .json({ message: "Błąd podczas aktualizacji notatki", details: error });
  }
});
...
router.get(
  ...
  async (req, res) => {
    try {
      ...
      res
        .status(500)
        .json({ message: "Błąd pobierania użytkowników", details: error });
    }
  }
);
...
router.delete(
  ...
  async (req, res) => {
    try {
      ...
      res
        .status(500)
        .json({ message: "Błąd podczas usuwania użytkownika", details: error });
    }
  }
);
...
router.post("/send-message", async (req, res) => {
  try {
    ...
    if (!token) {
      return res
        .status(401)
        .json({ message: "Brak tokena, nieautoryzowany dostęp" });
    }
...
router.patch("/message/:id/read", authMiddleware, async (req, res) => {
  try {
    ...
    res.status(500).json({
      message: "Błąd przy aktualizacji statusu",
      error: error.message
    });
  }
});
...
router.delete("/messages/:messageId", async (req, res) => {
  ...
  try {
    ...
    res
      .status(500)
      .json({ message: "Wystąpił błąd podczas usuwania wiadomości", error });
  }
});
...
router.get("/admin-contact", authMiddleware, async (req, res) => {
  try {
    ...
    res
      .status(500)
      .json({ message: "Błąd przy pobieraniu administratorów", error });
  }
});
